...
echo "================================================================"
echo "🚀 Installing channels['daphne']..."
pip install channels['daphne']

test=$(pip show channels['daphne'] > /dev/null 2>&1)
echo $test

if pip show channels['daphne'] > /dev/null 2>&1; then
    echo -e "✅ ${GREEN}channels['daphne'] is installed!${RESET}"
else
    echo -e "❌ ${RED}Failed to install channels['daphne']${RESET}"
    exit 1
fi
# Get the installed version of channels['daphne']
installed_version=$(pip show channels['daphne'] | grep Version | awk '{print $2}')
# Get the latest version of channels['daphne'] from PyPI
latest_version=$(pip install channels['daphne'] --upgrade | grep "Requirement already satisfied: channels['daphne'] in" | awk '{print $7}' | tr -d '()')

if [ $latest_version = $installed_version ]; then
    echo -e "✅ ${GREEN}channels['daphne'] is already up to date!${RESET}"
else
    echo -e "🔄 ${YELLOW}channels['daphne'] is outdated! Updating...${RESET}"
    pip install --upgrade channels['daphne']
    echo -e "🚀 ${YELLOW}channels['daphne'] updated to $latest_version!${RESET}"
fi
echo "================================================================"
